...
Artifact stores the common attributes of all SDLC artifacts - features, requirements, design, implementation, test cases etc.
*/
public class ArtifactDbAdapter extends BaseAdapter {
	

	private IReferenceMap reference;
	private ISecurity security;

	public ArtifactDbAdapter(IReferenceMap reference, ISecurity security) {
		this.reference = reference;
		this.security = security;
	}


	public UpdateResult UpdateArtifact(ArtifactModel m, ServiceCall call) throws DataUpdateException {
		DbConnection conn = null;
		try {
			conn = Db.getConnection();
			return UpdateArtifact(m, call, conn);
		} catch (Exception ex) {
	...
	public UpdateResult UpdateArtifact(ArtifactModel m, ServiceCall call, DbConnection conn) throws DataUpdateException {
		ArtifactModel original = null;
		try {
			if (m == null) {
			...
			if (m.hasModified()) {
				original = getArtifact(m.getArtifactId(), conn);
				...
				m.setArtifactId(original.getArtifactId());
				return result;
			} else {
	...
	private UpdateResult save(ArtifactModel m, DbConnection conn) throws SQLException, AccessDataTypeException {
		int nbrRows = 0;
		String sql = "";
		...
		try {
			if (m.getArtifactId()==0) {
				m.setArtifactId(Db.getNextSequence(TableNameFramework.ARTIFACT, conn));
				sql = getInsertSql();
			...
			return new UpdateResult(nbrRows,m.getArtifactId());
	...
	protected int ImportArtifact(ArtifactModel m) throws SQLException, AccessDataTypeException {
		int nbrRows = 0;
		String sql = "";
		SQLResult result = null;
		Vector params = null;
	...
	public int DeleteArtifact(ArtifactModel m, DbConnection conn) throws DataUpdateException {
		SQLResult result = null;
		try {
			int nbrRows = 0;
	 ...
	 */
	public void mapArtifact(ResultSet rset, ArtifactModel m) throws SQLException {
  ...
  */
	public ArtifactList  getArtifact(String sql) throws DataAccessException {
		DbConnection conn = null;
		try {
			conn = Db.getConnection();
			return getArtifact(sql, conn);
		} catch (Exception ex) {
  ...
  * Execute the SQL and return a list for the result set
  */
	public ArtifactList  getArtifact(String sql, DbConnection conn) throws DataAccessException {
		SQLResult result = null;
		try {
  ...
  * Execute the SQL and return a list for the result set
  */
	public ArtifactList  getArtifact(String sql, Vector params) throws DataAccessException {
		DbConnection conn = null;
		try {
			conn = Db.getConnection();
		return getArtifact(sql, params, conn);
		} catch (Exception ex) {
			throw new DataAccessException(ex);
		} finally {
			if (conn != null) {
  ...
  * Execute the SQL and return a list for the result set
  */
	public ArtifactList  getArtifact(String sql, Vector params, DbConnection conn) throws DataAccessException {
		SQLResult result = null;
		try {
			ArtifactList list = new ArtifactList();
	 ...
	 */
	public ArtifactModel getArtifact(int id) throws DataAccessException { 
		DbConnection conn = null;
		try {
			conn = Db.getConnection();
			return getArtifact(id, conn);
		} catch (Exception ex) {
			throw new DataAccessException(ex);
		} finally {
			if (conn != null) {
	 ...
	 * Returns an empty model if the id is 0 or not found
	 */
	public ArtifactModel getArtifact(int id, DbConnection conn) throws DataAccessException { 
		SQLResult result = null;
		try {
			ArtifactModel m = new ArtifactModel();
			...
			Debug.LogException(this, ex, "ArtifactModel" + id);
			throw new DataAccessException(ex);
		} finally {
			try { if (result!=null) result.closeStatements();} catch (Exception ex) {}
		}
	...
	public ArtifactList exportArtifact() throws DataAccessException {
		int rows = Db.countTable("artifact");
		Debug.LogInfo(this, rows + " found in artifact");
		String sql = "select * from artifact where record_type_ref_id in (?, ?)";
		ArtifactList list = this.getArtifact(sql, Db.getParameter(RecordTypeFramework.SYSTEMREFERENCE, RecordTypeFramework.USERREFERENCE)); 
		Debug.LogInfo(this,list.size()  + " artifact rows exported");
		return list;
	}

	public int importArtifact(ArtifactList list) throws Exception {
		Enumeration e1 = list.elements();
		int rows = 0;
		while (e1.hasMoreElements()) {
			ArtifactModel m = (ArtifactModel) e1.nextElement();
			rows += ImportArtifact(m);
		}
	...
	private String getSelectSql() {
		return "select artifact_id, product_ref_id, version_ref_id, artifact_ref_id, "
	...
	private String getInsertSql() {
		return "insert into artifact (artifact_id, product_ref_id, version_ref_id, artifact_ref_id, "
			+ "artifact_nbr, artifact_seq, artifact_level_ref_id, component_type_ref_id, "
			+ "last_updated_baseline_id, artifact_name, description, status_ref_id, "
			+ "priority_ref_id, complexity_ref_id, author_ref_id, assigned_ref_id, "
			+ "category_ref_id, assigned_user_id, assigned_user_group_id, effort, "
	...
	private Vector getInsertParameters(ArtifactModel m) {
	...
	private String getUpdateSql() {
		return "update artifact set  product_ref_id = ?, version_ref_id = ?, artifact_ref_id = ?, "
			+ "artifact_nbr = ?, artifact_seq = ?, artifact_level_ref_id = ?, component_type_ref_id = ?, "
			+ "last_updated_baseline_id = ?, artifact_name = ?, description = ?, status_ref_id = ?, "
			+ "priority_ref_id = ?, complexity_ref_id = ?, author_ref_id = ?, assigned_ref_id = ?, "
			+ "category_ref_id = ?, assigned_user_id = ?, assigned_user_group_id = ?, effort = ?, "
	...
	private Vector getUpdateParameters(ArtifactModel m) {
		Vector v = new Vector(60);
		v.add(new Integer(m.getProductRefId()));
		v.add(new Integer(m.getVersionRefId()));
		v.add(new Integer(m.getArtifactRefId()));
	...
	private Vector getDeleteParameters(ArtifactModel m) {
		Vector v = new Vector(60);
		v.add(new Integer(m.getArtifactId()));
		return v;
	}
	...
	private String getDeleteSql() {
		return "delete from artifact where artifact_id = ?";				 
	}
