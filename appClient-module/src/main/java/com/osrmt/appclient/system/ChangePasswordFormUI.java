...
null
*/
public class ChangePasswordFormUI extends UIJDialog {
	 ...
	 */
	public ChangePasswordFormUI(JFrame frame) {
		super(frame, false);
		initialize();
	}
	...
	public void addControls(ApplicationControlList controls) {
		...
		while (e1.hasMoreElements()) {
			...
			LayoutUtility.addControl(rm, "OldPassword", builder, rm.getControlText(), getOldPassword());
			LayoutUtility.addControl(rm, "NewPasswordConfirm", builder, rm.getControlText(), getNewPasswordConfirm());
			LayoutUtility.addControl(rm, "NewPassword", builder, rm.getControlText(), getNewPassword());
	...
	private UIPasswordField oldPassword = null;
	private UIPasswordField newPasswordConfirm = null;
	private UIPasswordField newPassword = null;


	public UIPasswordField getOldPassword() {
		if (this.oldPassword == null) {
			this.oldPassword = new UIPasswordField();
			this.oldPassword.setBackground(Color.YELLOW);
		}
		return oldPassword;
	}

	public void setOldPassword(UIPasswordField control) {
		this.oldPassword = control;
	}

	public UIPasswordField getNewPasswordConfirm() {
		if (this.newPasswordConfirm == null) {
			this.newPasswordConfirm = new UIPasswordField();
			this.newPasswordConfirm.setBackground(Color.YELLOW);
		}
		return newPasswordConfirm;
	}

	public void setNewPasswordConfirm(UIPasswordField control) {
		this.newPasswordConfirm = control;
	}

	public UIPasswordField getNewPassword() {
		if (this.newPassword == null) {
			this.newPassword = new UIPasswordField();
			this.newPassword.setBackground(Color.YELLOW);
		}
		return newPassword;
	}

	public void setNewPassword(UIPasswordField control) {
		this.newPassword = control;
	}
