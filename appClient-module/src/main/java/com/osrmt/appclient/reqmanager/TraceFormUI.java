...
import com.osframework.appclient.ui.controls.UIButton;
...
null
*/
public class TraceFormUI extends UIJDialog {
	 ...
	 */
	public TraceFormUI(JFrame frame) {
		super(frame, false);
		initialize();
	}
	...
	public void addControls(ApplicationControlList controls) {
		...
		while (e1.hasMoreElements()) {
			...
			LayoutUtility.addControl(rm, "TraceFrom", builder, rm.getControlText(), getTraceFrom());
			LayoutUtility.addControl(rm, "TraceType", builder, rm.getControlText(), getTraceType());
			LayoutUtility.addControl(rm, "TraceTo", builder, rm.getControlText(), getTraceTo());
			LayoutUtility.addControl(rm, "ApplyButton", builder, rm.getControlText(), getApplyButton());
	...
	private UIComboBox traceFrom = null;
	private UIComboBox traceType = null;
	private UIComboBox traceTo = null;
	private UIButton applyButton = null;


	public UIComboBox getTraceFrom() {
		if (this.traceFrom == null) {
			this.traceFrom = new UIComboBox();
			this.traceFrom.setBackground(Color.YELLOW);
		}
		return traceFrom;
	}

	public void setTraceFrom(UIComboBox control) {
		this.traceFrom = control;
	}

	public UIComboBox getTraceType() {
		if (this.traceType == null) {
			this.traceType = new UIComboBox();
			this.traceType.setBackground(Color.YELLOW);
		}
		return traceType;
	}

	public void setTraceType(UIComboBox control) {
		this.traceType = control;
	}

	public UIComboBox getTraceTo() {
		if (this.traceTo == null) {
			this.traceTo = new UIComboBox();
			this.traceTo.setBackground(Color.YELLOW);
		}
		return traceTo;
	}

	public void setTraceTo(UIComboBox control) {
		this.traceTo = control;
	}

	public UIButton getApplyButton() {
	...
	public void setApplyButton(UIButton control) {
