...
public class ProjectWizard {
	...
	private int selectUserGroupRefId = 0;

	public ProjectWizard(JFrame owner) throws HeadlessException {
		this.frame = owner;
	}
		...
		uiGroupList.addListSelectionListener(new UIListSelectionListener(frame){
			public void valueChangedExecuted(ListSelectionEvent e) throws Exception {
				...
				if (uiGroupList.getSelectedRow() instanceof ReferenceModel) {
					ReferenceModel ref = (ReferenceModel) uiGroupList.getSelectedRow();
					selectUserGroupRefId = ref.getRefId();
					selectedGroup = ref.getDisplay();
				}
			}
		});
	...
	public WizardScreen getScreen2() {
		return new WizardScreen(frame) {
			...
			public ActionListener getNextAction() {
				return new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						ReferenceTreeModel m = (ReferenceTreeModel) uiUserGroupList.getSelectedRow();
			...
			public String getTitle() {
				return ReferenceServices.getDisplay(FormTitleGroup.USERGROUPWIZARDSELECTPROJECT) + ": " + selectedGroup;
			}
	...
	public WizardScreen getScreen3() {
		return new WizardScreen(frame) {
			...
			public String getTitle() {
				return getSelectUserGroup().getTableRefDisplay();
			}
	...
	private ReferenceTreeModel getSelectUserGroup() {
		if (screen2.getUserObject() != null) {
			return (ReferenceTreeModel) screen2.getUserObject();
		} else {
	...
	private JPanel getUserGroupListPanel() {
		try {
			uiUserGroupList.setTableModel(getUserGroupProductList(), 60);
	...
	private JPanel getUserGroupForm() {
		...
		try {
			ApplicationControlList acl = SecurityServices.getAppControlsByUser(ApplicationFramework.get(ApplicationGroup.USERGROUPPRODUCT));
			ReferenceTreeModel rm = getSelectUserGroup();
			final ReferenceTreeModel refTree = new ReferenceTreeModel();
			//ReferenceServices.get(rm.getReferenceTreeId());
			JPanel panel = ControlPanel.getPanel(frame, acl, refTree);
			...
			applyPanel.getButtonPanel().getCmdButton0().addActionListener(new UIActionListener(frame){
				public void actionExecuted(ActionEvent e) throws Exception {
					//SecurityServices.updateApplicationUserGroup(refTree);
	...
	private ReferenceTreeList getUserGroupProductList() throws ResultColumnException {
		ReferenceTreeList list = ReferenceServices.getChildTrees(selectUserGroupRefId); 
		ApplicationControlList acl1 = SecurityServices.getAppControlsByUser(ApplicationFramework.get(ApplicationGroup.USERGROUPPROJECTSEARCHRESULTS));
	...
	private ActionListener addNewUserGroupListener(final MultiColumnList uiRefList) {
		...
		final ReferenceTreeModel refTreeProject = new ReferenceTreeModel();
		refTreeProject.setParentTableKeyId(getSelectedReferenceGroup().getRefId());
		return new UIActionListener(frame) {
			public void actionExecuted(ActionEvent e) throws Exception {
				...
				dialog.getButtonPanel().getCmdButton1().addActionListener(new UIActionListener(frame) {
					public void actionExecuted(ActionEvent e) throws Exception {
						ReferenceServices.updateReferenceTree(refTreeProject).getPrimaryKeyId();
						TimedAction action = new TimedAction(0.1) {
							public void executeTask() {
								try {
									uiRefList.setTableModel(getUserGroupProductList(), 60);
									screen2.initialize(screen1, screen3);
									screen2.invalidate();
									screen2.validate();
								} catch (Exception ex) {
//									Debug.LogException(this, ex);
				...
				dialog.getCenterPanel().add(buildAddUserGroupControlPanel(refTreeProject), BorderLayout.CENTER);
				dialog.pack();
	...
	private ActionListener addRemoveUserGroupListener(final MultiColumnList uiUserGroupList) {
		return new UIActionListener(frame) {
			public void actionExecuted(ActionEvent e) {
				try {
					final ReferenceTreeModel m = (ReferenceTreeModel) uiUserGroupList.getSelectedRow();
					if (!m.isNew()) {
						m.setNotActive();
	...
	private JPanel buildAddUserGroupControlPanel(ReferenceTreeModel rm) {
		try {
			ApplicationControlList list = SecurityServices.getAppControlsByUser(ApplicationFramework.get(ApplicationGroup.USERGROUPPRODUCT));
